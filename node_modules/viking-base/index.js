...
module.exports = {
  ...
  checkBowerDependencies: function() {

    // Move on to checking Bower dependencies
    checkDepends( this.checkDependsConfig.bower ).then(function( result ) {
  ...
  checkNpmDependencies: function() {
    ...
    // Check dependencies. First npm, then Bower.
    checkDepends( this.checkDependsConfig.npm ).then(function( result ) {

      // Check if npm dependencies were ok
      if ( result.depsWereOk ) {

        this.checkBowerDependencies.bind( this )();

      } else {

        // Check if we should automatically update dependencies
        if ( this.checkDependsConfig.autoUpdate ) {

        } else {

          // Prompt user to update npm dependencies
          ask('Node dependencies are out-of-date. Would you like to update them now? (y/n)', function( answer ) {

            answer = answer.toLowerCase();

            if ( answer.charAt(0) === 'y' || answer === 'true' || answer === '1' ) {
  ...
  preClean: function() {
    ...
    this.checkNpmDependencies.bind( this )();
  },
