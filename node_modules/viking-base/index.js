...
module.exports = {
  ...
  // Whether or not to use prod version of Ember for prod builds
  useProdEmber: true,
    ...
    try {
      ...
      this.checkDepends.bind( this )().then(function( results ) {
        ...
        if ( prompt && !this.watch ) {
          ...
          inter.question('Dependency updates are needed. Would you like to update all of them now? (Y/n)\n', function( answer ) {
  ...
  preproccessJs: function( buildFn ) {
    ...
    function extractScript( line ) {
      ...
      if ( matched ) {
        ...
        // Use prod version of Ember if option is enabled
        if ( this.prod && this.useProdEmber && matched[2].indexOf('ember.debug.js') !== -1 ) {

          return matched[2].replace('.debug', '.prod');
        }
    ...
    return function ( block ) {
      ...
      extractSrc = es.mapSync( extractScript.bind( this ) );
  ...
  // Hook for any additional processing needed of template file names
  processHbsFileName: function( fileName ) {
        ...
        return this.gulp.src( this.sources.hbs )
          ...
          .pipe( this.plugins.tap( function( file ) {
            ...
            if ( file.isBuffer() ) {
              ...
              // Test file path for back slashes (Windows)
              if ( regExp.test( path.sep ) ) {

                // Replace backslashes in file path with forward slashes
                fileName = this.processHbsFileName( fileName.replace( regExp, '/' ) );
              }

              file.contents = new Buffer( content.replace('export default', 'Ember.TEMPLATES["' + fileName + '"] =') );
            }
