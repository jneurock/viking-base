...
module.exports = {
  ...
  /*
   * A hash of dependencies for different gulp tasks
   */
  depends: {
    hbs: [
      'handlebars'
  ...
  // Instance of event-stream module
  es: null,
  // Instance of gulp module
  gulp: null,
  // Called by the gulp-htmlbuild plugin
  gulpSrc: function( opts, cb ) {

    var files = this.es.through(),
        paths = this.es.through();

    paths.pipe( this.es.writeArray(function( err, sources ) {
    ...
    return this.es.duplex( paths, files );
  },
  ...
  preproccessJs: function( buildFn ) {
    ...
    return function ( block ) {
      ...
      extractSrc = this.es.mapSync( extractScript );
      templateSrc = this.es.mapSync( templateScript );

      block.pipe( extractSrc );
      templateSrc.pipe( block );
      
      buildFn( this.es.duplex( templateSrc, extractSrc ) );
  ...
  tasks: {
    ...
    watch: {
      cb: function() {
        ...
        this.gulp.watch( this.sources.sass, this.depends.sass );

        this.gulp.watch( this.sources.hbs, this.depends.hbs );
      }
    }
  },
